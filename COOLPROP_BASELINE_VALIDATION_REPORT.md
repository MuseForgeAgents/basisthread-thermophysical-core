...
- Calls `MoveFileExW` directly with `MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH`. `MOVEFILE_WRITE_THROUGH` is retained as a conservative move flag, not as proof of filesystem durability for every same-volume rename — Microsoft specifically documents its flush guarantee for moves performed as copy/delete operations, which is a narrower claim than "every rename on this call is durable."
- Retries **only** on two error codes, on different grounds for each: `ERROR_SHARING_VIOLATION` is Microsoft's documented signature for a conflicting file-sharing/open-handle condition; `ERROR_ACCESS_DENIED` was empirically observed during exactly this concurrent same-target replacement race on this Windows environment, but it can also represent a persistent permission/security condition — retrying it is acceptable here because the retry is narrowly scoped and bounded (see below), and the error still propagates once the retry budget is exhausted. Any other error fails immediately, on the first attempt — **no blanket swallowing of `ERROR_ACCESS_DENIED`.**
...
| 4 | BasisThread engineering validation | ⬜ **NOT YET STARTED — CoolProp software verification is complete, but independent BasisThread known-answer engineering validation remains required.** The full documented baseline suite (§12.3 run 5) passing end-to-end demonstrates that CoolProp's *own* test suite is healthy on this Windows configuration; it is not itself BasisThread engineering validation. That work — comparing CoolProp's outputs for BasisThread's required water/glycol/humid-air/refrigerant states against independent known-answer references (§6–7 identify candidate sources; no values have been invented or checked against them yet) — has not been performed and remains a separate, required step. |
